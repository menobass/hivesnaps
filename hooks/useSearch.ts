...
export const useSearch = (): UseSearchReturn => {
  ...
  const saveToRecentHashtags = useCallback(async (hashtag: string) => {
    try {
  ...
  const removeRecentHashtag = useCallback(async (hashtag: string) => {
    try {
  ...
  const loadRecentSearches = useCallback(async () => {
    try {
      ...
      if (storedHashtags) {
        setState(prev => ({ ...prev, recentHashtags: JSON.parse(storedHashtags) }));
      }
    } catch (err) {
